---
description: Genera un nou episodi del podcast Can Gaietà (transcripció, capítols, soundbite, thumbnail, archive.org, deploy)
argument-hint: "[número-episodi opcional]"
---

# Generar Episodi del Podcast Can Gaietà
...
Els MP3 s'allotgen a **archive.org** (no a GitHub), acompanyats de la caràtula
de l'episodi perquè als llistats d'archive.org no hi surti la forma d'ona.

Si l'usuari ha indicat un número d'episodi com a argument, fes-lo servir. Si no, deriva'l automàticament al PAS PREVI.

---

## WORKFLOW COMPLET

Segueix aquests passos **en ordre estricte**. No saltis cap pas. Fes servir TodoWrite per fer el seguiment.
...
L'script crea `assets/thumbnails/XXX-nom-episodi.png` (1024×1024, ~1,3 MB) i mostra
la línia per afegir al frontmatter.
...
**Versió reduïda per als llistats web** (`assets/thumbnails/small/XXX-nom-episodi.webp`,
400×400, ~30 KB): la genera automàticament el PAS 8. Si la vols abans:

```bash
python scripts/generate_thumbnails_small.py XXX
```

⚠️ **Aquest pas no és opcional.** Sense caràtula, el PAS 8 s'atura i no puja res.
...
### PAS 8: Pujar MP3 i caràtula a archive.org
...
Això:
- **Comprova que hi hagi caràtula.** Si falta, s'atura, NO puja res i surt amb codi 1
- **Genera la versió reduïda** (`small/XXX-nom-episodi.webp`) si falta
- Puja l'MP3 **i la caràtula** (com a `XXX-nom-episodi-cover.png`) al mateix upload
...
- Verifica contra l'API de metadades que la caràtula hi hagi arribat

**Si falla per rate limit:** informar l'usuari que cal esperar 30-60 minuts.

**Si s'atura per falta de caràtula:** tornar al PAS 5d. No fer servir
`--permet-sense-caratula` sense demanar-ho a l'usuari.

**Si la caràtula no es confirma** (l'MP3 sí que ha pujat):
```bash
python scripts/upload_to_archive.py --nomes-cover --episodi XXX
```
És idempotent i no torna a pujar l'àudio.

Verificar que `audio_file` al markdown s'ha actualitzat correctament. Si no, actualitzar-lo manualment.

**⚠️ Les dues trampes d'archive.org** (documentades a CLAUDE.md): un 200 d'`upload()`
no vol dir que el fitxer hi hagi arribat, i archive.org genera un derivat anomenat
igual que la caràtula (`XXX-nom-episodi.png`) que és la forma d'ona. **Mai** verificar
amb un HEAD: cal l'API de metadades exigint `source == "original"` i mida coincident.
L'script ja ho fa; no el "simplifiquis".
...
11. ✅ La versió reduïda existeix a `assets/thumbnails/small/XXX-nom-episodi.webp`
12. ✅ La caràtula és a archive.org, confirmada amb l'API de metadades:
    ```bash
    curl -s https://archive.org/metadata/podcast-cangaieta-XXX-nom-episodi \
      | python3 -c "import json,sys; print([f for f in json.load(sys.stdin)['files'] if f['name'].endswith('-cover.png')])"
    ```
    Ha de sortir `'source': 'original'` i la mida del PNG local.
13. ✅ El contingut del body és correcte (introducció, temes, fonts, disclaimer)
14. ✅ `scripts/upload_to_archive.py` conté el nou episodi a la llista
...
**SEMPRE fer commit i push automàticament** al final de generar l'episodi. NO cal demanar confirmació: és l'últim pas obligatori del workflow i s'ha d'executar sempre.
...
git add _episodes/XXX-nom-episodi.md \
        ...
        assets/thumbnails/small/XXX-nom-episodi.webp \
...
- La versió reduïda va en un subdirectori: el glob `assets/thumbnails/XXX-*.*`
  NO l'agafa, cal la línia `small/` explícita
...
- **Deploy**: SEMPRE fer `git commit` i `git push` al final, automàticament i sense demanar confirmació. Generar un episodi sense fer commit+push es considera incomplet.
